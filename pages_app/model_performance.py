...
from utils.data import get_model_performance, get_matchup_edge_history, get_prediction_log
...
PLOT_DEFAULTS = dict(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
...
def render():
    ...
    with tab1:
        ...
        if not log:
            ...
            c3.metric("Bet Accuracy (>3% edge)", f"{betted_acc*100:.1f}%",
                      help="Win rate when model had >3% edge over DK")
            c4.metric("Cumulative ROI",     f"{cum_roi:+.2f}u",
            ...
            fig_roll.add_trace(go.Scatter(
                ...
                marker=dict(
                    color=[GREEN if r >= 0.60 else (ORANGE if r >= 0.50 else RED)
                           for r in rolling_acc],
                    size=7,
                ),
            ...
            fig_roll.add_hline(y=60, line_dash="dot", line_color=GREEN,
                               annotation_text="60% target", opacity=0.6)
            ...
            if len(cum) > 0:
                ...
                fig_cum = go.Figure(go.Scatter(
                    ...
                    mode="lines",
                    fill="tozeroy",
                    line=dict(color=GREEN if final >= 0 else RED, width=2),
                    fillcolor="rgba(46,204,113,0.12)" if final >= 0
            ...
            st.caption("Every prediction the model made, with the actual result reconciled automatically after the game.")

            col_f1, col_f2, col_f3 = st.columns(3)
            with col_f1:
                result_filter = st.selectbox(
                    "Result", ["All", "Correct only", "Wrong only"])
            with col_f2:
            ...
            if team_filter != "All":
                log_view = log_view[
                    (log_view["team1"] == team_filter) |
                    (log_view["team2"] == team_filter)
                ]

            display_log = log_view[[
                "date", "team1", "team2", "model_pick",
                "model_pick_prob", "dk_implied", "edge",
                "actual_winner", "correct",
                "predicted_total", "dk_total_line", "actual_total",
                "total_correct", "roi_winner",
            ]].copy()

            display_log.columns = [
                "Date", "Team 1", "Team 2", "Model Pick",
                "Model Prob", "DK Implied", "Edge",
                "Actual Winner", "Correct?",
                "Pred Total", "DK Line", "Actual Total",
                "Total ✓?", "ROI (u)",
            ]
            ...
            display_log["DK Implied"]  = display_log["DK Implied"].apply(
                lambda x: f"{x*100:.1f}%" if pd.notna(x) else "–")
            display_log["Edge"]        = display_log["Edge"].apply(
                lambda x: f"{x*100:.1f}%" if pd.notna(x) else "–")
            display_log["ROI (u)"]     = display_log["ROI (u)"].apply(
                lambda x: f"{x:+.3f}" if pd.notna(x) else "no bet")
    ...
    with tab6:
        ...
        if not filtered:
            ...
            df_m.columns = ["Matchup", "Games", "Avg Edge", "Win Rate (edge>0)",
    ...
    with tab7:
        ...
        df_v.columns = ["Venue", "Surface", "Games", "Avg Edge",
                        "Winner ROI", "Totals ROI", "1st Inn. Error", "Best Bet"]
