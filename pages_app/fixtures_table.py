...
from utils.data import get_ipl_schedule, get_points_table, get_playoff_probabilities
...
def render():
    ...
    with tab3:
        ...
        with inner_tab1:
            ...
            colors = [
                QUALIFY_GREEN  if p >= 0.60 else
                WARNING_ORANGE if p >= 0.35 else
                DANGER_RED
            ...
            # Summary table
            summary = tr_df[[
                "team", "current_pts", "games_remaining",
                "qualify_prob", "avg_finish", "avg_final_pts",
            ]].copy()
            summary.columns = [
                "Team", "Current Pts", "Games Left",
                "Qualify %", "Avg Finish", "Avg Final Pts",
            ]
        ...
        with inner_tab2:
            ...
            combined = pd.DataFrame(team_results)[[
                "team", "qualify_prob", "title_prob", "current_pts",
                "games_remaining", "avg_finish"
            ]].copy()
            combined.columns = [
                "Team", "Qualify %", "Title %", "Pts", "Games Left", "Avg Finish"
            ]
        ...
        with inner_tab3:
            ...
            st.caption(
                "Stacked bars show how often each team finishes in each position "
                "across all simulations."
            )

            teams_list  = [r["team"] for r in team_results]
            pos_colors  = [QUALIFY_GREEN, "#27ae60", WARNING_ORANGE, "#e67e22",
                           DANGER_RED, "#c0392b", "#8e44ad", "#2980b9",
                           "#1abc9c", "#95a5a6"]

            fig3 = go.Figure()
            for pos in range(1, 6):
                probs = [
                    r["position_dist"].get(f"P{pos}", 0)
            ...
            if team_row:
                ...
                pos_df = pd.DataFrame([
                    {"Position": f"#{k[1:]}", "Probability": f"{v:.1f}%"}
        ...
        with inner_tab4:
            ...
            if not match_importance:
                ...
                fig4 = go.Figure(go.Bar(
                    ...
                    marker_color=[
                        QUALIFY_GREEN  if r["importance"] > 0.15 else
                        WARNING_ORANGE if r["importance"] > 0.08 else
                        NEUTRAL_BLUE
                ...
                # Full table
                mi_display = mi_df[[
                    "match", "date", "team1", "team2", "importance", "t1_swing", "t2_swing"
                ]].copy()
                mi_display.columns = [
                    "#", "Date", "Team 1", "Team 2",
                    "Importance", "T1 Qualify Swing", "T2 Qualify Swing"
                ]
