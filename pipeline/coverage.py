...
def competition_status(competition: Competition, *, fixtures: list[dict], odds: list[dict],
                       historical: dict | None = None, model_ready: bool | None = None,
                       qualifying_bets: int = 0, fetch_error: str | None = None) -> dict:
    ...
    dk_rows = [o for o in odds_rows if o.get("event_status") == "draftkings_available"
...
def build_status_report(fixtures: list[dict], odds: list[dict], historical_by_competition: dict | None = None,
                        model_ready_by_competition: dict | None = None,
                        bets_by_competition: dict | None = None, errors: dict | None = None) -> dict:
    ...
    return {"schema_version": 1, "generated_at": datetime.now(timezone.utc).isoformat(), "competitions": rows}
...
def inspect_historical_coverage(competition: Competition, raw_dir: Path) -> dict:
    ...
    if not path.exists():
        return {"path": str(path), "seasons": [], "completed_matches": 0,
                "team_identity_rate": 0.0, "player_identity_rate": 0.0, "ready": False}
    try:
        import pandas as pd
        ...
        return {"path": str(path), "seasons": seasons, "completed_matches": matches,
                "team_identity_rate": round(team_rate, 4),
                "player_identity_rate": round(player_rate, 4),
                "ready": len(seasons) >= 2 and matches >= MIN_COMPLETED_MATCHES
                and team_rate >= MIN_TEAM_IDENTITY_RATE and player_rate >= MIN_PLAYER_IDENTITY_RATE}
    except Exception as exc:
        return {"path": str(path), "seasons": [], "completed_matches": 0,
                "team_identity_rate": 0.0, "player_identity_rate": 0.0,
