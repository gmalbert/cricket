...
def predict_match_winner(match_features: list[dict]) -> list[dict]:
    ...
    if training_df is None or len(training_df) < 50:
        logger.warning("Insufficient training data (%s rows), using fallback",
                       len(training_df) if training_df is not None else 0)
        return _fallback_match_winner(match_features)
...
def _build_match_training_set(bbb: pd.DataFrame) -> pd.DataFrame | None:
    ...
    required = {"match_id", "innings", "batting_team", "bowling_team",
                "runs_off_bat", "extras", "venue", "start_date"}
    ...
    matches = inn1.merge(
        inn2[["match_id", "batting_team", "total"]].rename(
            columns={"batting_team": "team2", "total": "team2_score"}
        ),
