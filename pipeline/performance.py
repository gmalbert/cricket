...
def calculate_calibration(predictions: list[dict[str, Any]], n_bins: int = 10) -> dict[str, Any]:
    ...
    for i in range(n_bins):
        ...
        bin_preds = [p for p in predictions if bin_min <= p.get("model_pick_prob", 0) < bin_max]
...
def performance_by_competition(predictions: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    return {comp: calculate_performance_summary(predictions, competition=comp) for comp in competitions}
