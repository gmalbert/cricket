...
from pipeline.normalization import canonical_team, canonical_player, canonical_text
...
def match_fixture_to_odds(
    ...
    for odds_record in odds:
        ...
        for (ft1, ft2, ot1, ot2) in [
            ...
            # Substring match
            elif (canonical_team(ft1).lower() in canonical_team(ot1).lower() and
                  canonical_team(ft2).lower() in canonical_team(ot2).lower()):
                confidence = 0.85
                if confidence > best_confidence:
                    best_match = {**odds_record, "match_confidence": confidence}
                    best_confidence = confidence
